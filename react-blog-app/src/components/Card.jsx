...
const Card = ({ post }) => {
  ...
  return (
    <div className='flex flex-col shadow rounded-2xl shadow-slate-500 p-2 gap-2'>
      ...
      <p>
        By <span className='text-red-700 font-semibold'>{post.author}</span> on <span className='text-purple-900 font-semibold cursor-pointer' onClick={() => navigate(`/tags/${post.category}`)}>{post.category}</span>
      </p>
